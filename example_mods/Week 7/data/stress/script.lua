...
function makeChart()
	...
	for i = 0, len do
		if getPropertyFromGroup('eventNotes', i, 1) == 'Tankman Note' then
			isRight = true;
			if getPropertyFromGroup('eventNotes', i, 2) == 'left' then
				isRight = false;
			end
	...
	for i = 0, len do
		if getPropertyFromGroup('eventNotes', len-i, 1) == 'Tankman Note' then
...
function onUpdate(elapsed)
	...
	if curPicoNote < maxTankman and chartTankman[curPicoNote].time <= getSongPosition() then
		...
		math.randomseed(os.time() + getSongPosition() / 100);
