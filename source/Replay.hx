...
class Replay
{
    ...
    public function new(?usePlayStateVars:Bool = true)
    {
        if(usePlayStateVars)
        {
            ...
            difficulty = PlayState.storyDifficulty.toLowerCase();
        }

        ghostTapping = options.Option.getData("ghostTapping");
        offset = Conductor.offset;
        judgementTimings = options.Option.getData("judgementTimings");
        antiMash = options.Option.getData("antiMash");
