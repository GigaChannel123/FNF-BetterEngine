...
class DialogueEditorState extends MusicBeatState
{
	...
	function updateTextBox() {
		...
		DialogueBoxBetterEngine.updateBoxOffsets(box);
	}
	...
	function reloadCharacter() {
		...
		character.x = DialogueBoxBetterEngine.LEFT_CHAR_X;
		character.y = DialogueBoxBetterEngine.DEFAULT_CHAR_Y;

		switch(character.jsonFile.dialogue_pos) {
			case 'right':
				character.x = FlxG.width - character.width + DialogueBoxBetterEngine.RIGHT_CHAR_X;
	...
	function reloadText(speed:Float = 0.05) {
		...
		daText = new Alphabet(DialogueBoxBetterEngine.DEFAULT_TEXT_X, DialogueBoxBetterEngine.DEFAULT_TEXT_Y, textToType, false, true, speed, 0.7);
		add(daText);
