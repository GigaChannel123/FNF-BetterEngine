...
import StageData;
import FunkinLua;
import DialogueBoxBetterEngine;
import Note;
import Shaders;
...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		var file:String = Paths.json(songName + '/dialogue'); //Checks for json/Better Engine dialogue
		if (OpenFlAssets.exists(file)) {
			dialogueJson = DialogueBoxBetterEngine.parseDialogue(file);
		}
  ...
  public function clearShaderFromCamera(cam:String){
	...
	public var betterDialogue:DialogueBoxBetterEngine;
	//You don't have to add a song, just saying. You can just do "startDialogue(dialogueJson);" and it should work
	public function startDialogue(dialogueFile:DialogueFile, ?song:String = null):Void
	{
		// TO DO: Make this more flexible, maybe?
		if(betterDialogue != null) return;

		if(dialogueFile.dialogue.length > 0) {
			inCutscene = true;
			CoolUtil.precacheSound('dialogue');
			CoolUtil.precacheSound('dialogueClose');
			betterDialogue = new DialogueBoxBetterEngine(dialogueFile, song);
			betterDialogue.scrollFactor.set();
			if(endingSong) {
				betterDialogue.finishThing = function() {
					betterDialogue = null;
					endSong();
				}
			} else {
			...
			betterDialogue.nextDialogueThing = startNextDialogue;
			betterDialogue.skipDialogueThing = skipDialogue;
			betterDialogue.cameras = [camHUD];
			add(betterDialogue);
		} else {
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			//Change Stage Event Canceled, im not too smart at haxe :p 
			/*case 'Change Stage':
				switch(curStage) {
					case 0 :
						if(curStage != coolswag) then
							sus
				}*/
